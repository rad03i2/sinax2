# -*- coding: utf-8 -*-
"""
SINAX Image Tool Card Widget
Fluent UI tool card for Image Center catalog featuring icon, titles,
batch capability tags, favorite toggle, and interactive hover styling.
"""
...
class ImageCardWidget(QFrame):
    """Interactive card representing an image processing tool in the dashboard."""

    clicked = Signal(str)            # Emits tool_id when opened
    favorite_toggled = Signal(str, bool) # tool_id, is_favorite

    def __init__(self, tool: ImageToolDefinition, is_favorite: bool = False, parent=None):
    ...
    def _init_ui(self):
        ...
        icon_box.setStyleSheet("""
            QFrame {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #103B60, stop:1 #0078D4);
                border-radius: 8px;
            }
        """)
        ...
        # Badges row (Batch tag + Status badge)
